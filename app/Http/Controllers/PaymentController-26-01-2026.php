    ...
    public function createPayout(Request $request)
    {
        ...
        try {
            ...
            if (!$user) {
                ...
                $user = User::create([
                    'name' => $request->requisites['customer']['first_name'] . ' ' . $request->requisites['customer']['last_name'],
                    'email' => $customerEmail,
                    'password' => Hash::make($randomPassword),
                    'role' => 'user',
                ]);
                
                LoggingService::logActivity('Payment', 'auto_create_user', [
                    'user_id' => $user->id,
                    'email' => $user->email,
                    'created_by' => auth()->id(),
                ], auth()->id());
                
                \Log::info('Auto-created user for transaction', [
                    'user_id' => $user->id,
                    'email' => $user->email,
                    'created_by' => auth()->id(),
                ]);
            ...
            // Build customer data from found/created user
            $ipAddress = $this->getClientIpAddress($request);
            
            $payload['customer'] = [
                'id' => (string) $user->id, // Use customer user's ID
                'email' => $user->email,     // Use customer user's email address
                'ip_address' => $ipAddress,   // Automatically detected IP
            ];
            ...
            $transaction = Transaction::create([
                'user_id' => $user->id, // Use customer user's ID, not the authenticated admin/user
                'merchant_order_id' => $request->merchant_order_id,
